...
void Haffman(int weight[], int n, HaffNode haffTree[]) //建立叶结点个数为n权值为weight的哈夫曼树haffTree 　　
{
	int j, m1, m2, x1, x2; //哈夫曼树haffTree初始化。n个叶结点的哈夫曼树共有2n-1个结点 　　
...
int main(void)
{
	int i, j, n = 4;
	int weight[] = {1, 3, 5, 7};
	HaffNode *myHaffTree = (HaffNode *)malloc((2*n+1)*sizeof(HaffNode));
	Code *myHaffCode =(Code *) malloc(n*sizeof(Code));
	if (n > MaxN) {
